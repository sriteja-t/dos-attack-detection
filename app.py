...
st.title("Top request URLs")
url_counts = dict(df["request_url"].value_counts())
keys = url_counts.keys()
vals = url_counts.values()
new_dict = {k:v for (k, v) in zip(keys, vals) if not k.endswith((".ico", ".jpg", ".jpeg", ".png", ".css", "/"))}
new_dict = dict(itertools.islice(new_dict.items(), 10))
requestURLs = pd.DataFrame(new_dict.values(), new_dict.keys())
st.bar_chart(requestURLs)
